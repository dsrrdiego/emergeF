...
function cargarRocola(idAlbum, refresh) {
    fetch(direccionApi + '/canciones/' + idAlbum)
        .then(response => {
            if (!response.ok) {
                alert("no hay conexion, el servidor puede tardar hasta 1 minuto para encenderse");
                return
            }
        ...
        .then(respuesta => {
            if (respuesta.length == 0) {
                discosDiv.innerHTML = '<h1>Parece que todavia no hay discos subidos, o hay algun problema de conexion, El servidor puede tardar unos 60s en iniciarse</h1>'
